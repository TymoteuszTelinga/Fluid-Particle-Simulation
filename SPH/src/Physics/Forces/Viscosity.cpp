...
void Viscosity::Apply(Ref<Particles> particles, float deltaTime) {
	...
	ViscosityCuda(p_spec.KernelRange, kernel->Poly6Factor, p_spec.ViscosityStrength, particles->getSize(), cellRows, cellCols,
		particles->c_indices_addr, particles->c_indices_size, deltaTime);
}
